...
import com.myhotel.patterns.stragegy.DeluxeRoom;
import com.myhotel.patterns.stragegy.RoomContext;
import com.myhotel.patterns.stragegy.RoomFactory;
import com.myhotel.patterns.stragegy.StandardRoom;
import com.myhotel.patterns.stragegy.SuiteRoom;
...
public class CheckInController extends ApplicationController implements Initializable {
	...
	RoomFactory standard = new StandardRoom();
	RoomFactory deluxe = new DeluxeRoom();
	RoomFactory suite = new SuiteRoom();
	...
	@FXML
	private Label lbStandardRevenue,lbDeluxeRevenue,lbSuiteRevenue;
...
//			userMess.setText("Hello " + currentUser.getFirstName() + " !");
	...
	public void loadBookingTable() {
		...
		loadRevenue(bookings);

	}

	public void loadRevenue(List<Booking> bookings) {
		for (Booking booking : bookings) {
			...
			for (Room room : rooms) {
				...
				} else if (room.getRoomType() == RoomType.Suite) {
					suiteRooms.add(room);
				}
			}
		}
		lbStandardRevenue.setText("$" + RoomContext.getInstance(standard).getRevenue(standardRooms));
		lbDeluxeRevenue.setText("$" + RoomContext.getInstance(deluxe).getRevenue(deluxeRooms));
		lbSuiteRevenue.setText("$" + RoomContext.getInstance(suite).getRevenue(suiteRooms));
	}
	...
	private void setColumnProperties() {
		...
		colCheckIn.setCellValueFactory(cellData -> {
			boolean checkIn = cellData.getValue().getCheckInStatus() == null ? false
					: cellData.getValue().getCheckInStatus();
		...
		colCheckOut.setCellValueFactory(cellData -> {
			boolean checkOut = cellData.getValue().getCheckOutStatus() == null ? false
					: cellData.getValue().getCheckOutStatus();
